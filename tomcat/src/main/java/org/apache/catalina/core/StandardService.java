...
public class StandardService extends LifecycleMBeanBase implements Service {
	 ...
	 * org.apache.catalina.startup.Embedded subclass, this holds the most recently
	 * added Engine.)
	 */
			...
			for (Connector connector : connectors) {
				try {
					...
					if (connector.getState() != LifecycleState.FAILED) {
						// 调用Connector的init方法
						// 调用LifecycleBean.start()-->abstract startInternal()-->Connector.startInternal()
						connector.start();
					}
				} catch (Exception e) {
					log.error(sm.getString("standardService.connector.startFailed", connector), e);
	...
	protected void initInternal() throws LifecycleException {
		...
		synchronized (connectors) {
			for (Connector connector : connectors) {
				try {
					// 调用Connector的init方法
					// 调用LifecycleBean.init()-->abstract initInternal()-->Connector.initInternal()
					connector.init();
				} catch (Exception e) {
