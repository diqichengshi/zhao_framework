...
public abstract class ContainerBase extends LifecycleMBeanBase implements Container {
	...
	/**
	 * Perform addChild with the permissions of this class. addChild can be called
	 * with the XML parser on the stack, this allows the XML parser to have fewer
	 * privileges than Tomcat.
	 */
	...
	/**
	 * The access log to use for requests normally handled by this container that
	 * have been handled earlier in the processing chain.
	 */
	...
	/**
	 * Get the delay between the invocation of the backgroundProcess method on this
	 * container and its children. Child containers will not be invoked if their
	 * delay value is not negative (which would mean they are using their own
	 * thread). Setting this to a positive value will cause a thread to be spawn.
	 * After waiting the specified amount of time, the thread will invoke the
	 * executePeriodic method on this container and all its children.
	 */
	@Override
	...
	/**
	 * Set the delay between the invocation of the execute method on this container
	 * and its children.
	 * 
	 * @param delay The delay in seconds between the invocation of backgroundProcess
	 *              methods
	 */
	@Override
	...
	/**
	 * Return descriptive information about this Container implementation and the
	 ...
	 * associated Loader, return the Loader associated with our parent Container (if
	...
	/**
	 * Return the Manager with which this Container is associated. If there is no
	 * associated Manager, return the Manager associated with our parent Container
	...
	/**
	 * Return the Cluster with which this Container is associated. If there is no
	 * associated Cluster, return the Cluster associated with our parent Container
	 * (if any); otherwise return <code>null</code>.
	 */
	@Override
	 ...
	 * Within the set of child containers belonging to a particular parent,
	 * Container names must be unique.
	 *
	 * @param name New name of this container
	 *
	 * @exception IllegalStateException if this Container has already been added to
	 *                                  the children of a parent Container (after
	 *                                  which the name may not be changed)
	 */
	@Override
	...
	/**
	 * Return if children of this container will be started automatically when they
	...
	/**
	 * Set if children of this container will be started automatically when they are
	...
	/**
	 * Return the Container for which this Container is a child, if there is one. If
	 * there is no defined parent, return <code>null</code>.
	 */
	@Override
	 ...
	 * This Container may refuse to become attached to the specified Container by
	 ...
	 * is meaningful only <strong>after</strong> a Loader has been configured.
	 */
	@Override
	...
	/**
	 * Set the parent class loader (if any) for this web application. This call is
	 * meaningful only <strong>before</strong> a Loader has been configured, and the
	 * specified value (if non-null) should be passed as an argument to the class
	 * loader constructor.
	 *
	 *
	 ...
	 * associated Realm, return the Realm associated with our parent Container (if
	 * any); otherwise return <code>null</code>.
	 */
	@Override
	 ...
	 * associated. If there is no associated resources object, return the resources
	 ...
	 * supported. Prior to adding this Container to the set of children, the child's
	 * <code>setParent()</code> method must be called, with this Container as an
	 * argument. This method may thrown an <code>IllegalArgumentException</code> if
	 * this Container chooses not to be attached to the specified Container, in
	 * which case it is not added
	 *
	 ...
	 *                                     child Container
	 * @exception IllegalArgumentException if the new child does not have a name
	 *                                     unique from that of existing children of
	 *                                     this Container
	 * @exception IllegalStateException    if this Container does not support child
	 *                                     Containers
	 */
	@Override
	...
	/**
	 * Return the set of children Containers associated with this Container. If this
	...
	/**
	 * Return the set of container listeners associated with this Container. If this
	 * Container has no registered container listeners, a zero-length array is
	 * returned.
	 */
	@Override
	 ...
	 *
	 * @exception IllegalStateException if neither a pipeline or a basic Valve have
	 *                                  been configured for this Container
	 * @exception IOException           if an input/output error occurred while
	 *                                  processing
	 * @exception ServletException      if a ServletException was thrown while
	 ...
	 * parent. If there is no parent and still none is found, use the NoOp access
	 * log.
	 */
	@Override
	...
	/**
	 * Convenience method, intended for use by the digester to simplify the process
	 * of adding Valves to containers. See {@link Pipeline#addValve(Valve)} for full
	 * details. Components other than the digester should use
	 * {@link #getPipeline()}.{@link #addValve(Valve)} in case a future
	 * implementation provides an alternative method for the digester to use.
	 *
	...
	/**
	 * Notify all container event listeners that a particular event has occurred for
	...
	/**
	 * Private thread class to invoke the backgroundProcess method of this container
