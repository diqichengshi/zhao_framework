...
public class Catalina {
	...
	/**
	 * Start a new server instance. load方法做的事情如下：
	 * 
	 * 初始化目录，还有embedded的目录处理。 初始化命名空间。
	 * 创建并配置一个Digester实例，并设置相关规则属性，Digester用来解析xml，采用的是SAX方式。 读取解析配置文件。
	...
	public void load() {
		...
		try {
			// 对Server进行初始化(LifecycleBase中的init方法)
			getServer().init();
		} catch (LifecycleException e) {
